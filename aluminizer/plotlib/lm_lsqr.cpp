...
// LM_LeastSquare
//=============================================================================
LM_LeastSquare::LM_LeastSquare(int m, const std::vector<double>& x, numerics::FitObject* pFitObject)	:
	m_m(m),
	m_x(x),
	m_pFitObject(pFitObject)
{
    m_nan   = 0;
    m_info  = 0;
    m_fnorm = 0;
    ...
    m_n = static_cast<int>(m_x.size());
    m_ldfjac = m_m;

	m_fvec = new double [m_m];
    m_diag = new double [m_n];
    m_fjac = new double [m_n * m_m];
    m_qtf  = new double [m_n];
    m_ipvt = new int [m_n];
...
#if BUG
    int iflag = 1;
    fcn(m_m, m_n, &(m_x[0]), m_fvec, &iflag);
    printf( "initial x\n" ); pmat( 1, m_n, &(m_x[0]) ); // display 1 by n matrix
    //printf( "initial function\n" ); pmat( 1, m, fvec );
#endif
    
    lmdif(m_m, m_n, &(m_x[0]), m_fvec, m_ftol, m_xtol, m_gtol, m_maxfev, m_epsfcn,
...
std::string LM_LeastSquare::GetResultMessage() const
{
	std::string msg;
    ...
    switch (m_info)
    {
        ...
        case 1 : msg = "Both actual and predicted relative reductions in the sum of squares are at most ftol (%lg)."; break;
        case 2 : msg = "Relative error between two consecutive iterates is at most xtol (%lg)."; break;
        case 3 : msg = "Both actual and predicted relative reductions in the sum of squares are at most ftol (%lg).";
                 msg += "Relative error between two consecutive iterates is at most xtol (%lg)."; break;
        case 4 : msg = "The cosine of the angle between fvec and any column of the jacobian is at most gtol (%lg) in absolute value."; break;
        case 5 : msg = "Number of iterations has reached or exceeded %d, try adjusting initial values."; break;
        case 6 : msg = "ftol (%lg) is too small. no further reduction in the sum of squares is possible"; break;
        case 7 : msg = "xtol (%lg) too small. no further improvement in approximate solution x possible"; break;
        case 8 : msg = "gtol (%lg) is too small. fvec is orthogonal to the columns of the jacobian to machine precision."; break;
        default : break;
    }
    
    if (m_nan)
        msg += " %ld values were skipped since they were NaN";
...
void LM_LeastSquare::fcn(int, int, double x[], double fvec[], int *)
{
   m_pFitObject->fcn(x, fvec);
}
...
void LM_LeastSquare::qrfac(int m, int n, double a[], int, int pivot, int ipvt[], 
...
void LM_LeastSquare::fdjac2(int m,int n, double x[], double fvec[], double fjac[], 
                            int, int *iflag, double epsfcn, double wa[])
{
    int i,j,ij;
    double eps,h,temp;
    static double zero = 0.0;
