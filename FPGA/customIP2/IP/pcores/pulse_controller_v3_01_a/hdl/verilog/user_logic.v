...
//----------------------------------------------------------------------------
//
// Communicates with the embedded processor by a combination of a FIFO buffer and eight 32 bit registers
//
// slv_reg0			(input)	- dds controller commands
// slv_reg1[0]		(input)	- PMT correlation config data write
// slv_reg1[1:5]	(input)	- PMT correlation config data
// slv_reg2[0]		(output) - underflow_out
// slv_reg2[1]		(output) - correlation_data_ready
// slv_reg3			(output) - correlation_data_out[0:63]
// slv_reg4			(output) - correlation_data_out[64:127]
// slv_reg5			(output) - correlation_data_out[128:191]
// slv_reg6			(output) - correlation_data_out[192:255]
// slv_reg7			() 		- unused
...
wire [0:255] correlation_data_out;
    ...
    begin: SLAVE_REG_WRITE_PROC
      ...
      if ( Bus2IP_Reset == 1 )
        ...
        slv_reg2[0] <= underflow_out;
		  ...
		  // slv_reg3 <= 64'h0000000100020003;
		  // slv_reg4 <= 64'h0004000500060007;
		  // slv_reg5 <= 64'h00080009000A000B;
		  // slv_reg6 <= 64'h000C000D000E000F;
		  slv_reg3 <= correlation_data_out[0:63];
		  slv_reg4 <= correlation_data_out[64:127];
		  slv_reg5 <= correlation_data_out[128:191];
		  slv_reg6 <= correlation_data_out[192:255];
...
timing_controller #(.N_DDS(N_DDS), .N_COUNTER(N_COUNTER), .iFIFO_WIDTH(C_SLV_DWIDTH), .rFIFO_WIDTH(C_SLV_DWIDTH))
						tc(.clock(Bus2IP_Clk), .reset(Bus2IP_Reset), 
						   ...
						   .dds_data_T(dds_data_T), .dds_control(dds_control), .dds_cs(dds_cs),
							...
							.counter_in(counter_in), .sync_in(sync_in),
							.correlation_config_in(slv_reg1[0:5]), .correlation_data_out(correlation_data_out), .correlation_data_ready(correlation_data_ready));
